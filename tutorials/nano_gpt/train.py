...
def batch(batch_size: int, context_length: int, data_type: str = "train") -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    - Keep in mind that input sequence will be a list of tokens (a tensor of tokens) with a fixed `context_length` that are randomly picked from the input data. There will be `batch_size` such sequences making up the entire batch of X-token-sequences
...
def train(model: torch.nn.Module, epochs: int, max_num_tokens: int, batch_size: int, context_length: int):
    ...
    print(tokenizer.decode(model.generate(
        context, max_num_tokens)[0].tolist()))


model = llm.BigramLanguageModel(
    vocab_size, embedding_dim, context_length).to(DEVICE)
