...
def calculate_bandwidth(tensors: list, duration: float) -> float:
    total_mb = sum(float(t.numel() * t.element_size()) / (1024**2)
                   for t in tensors)
...
def format_results(results: Dict[str, Any], duration: float, bandwidth: float) -> None:
    ...
    print(
        f"Tokens: {results['token_count']} | Splitting: {'Yes' if results.get('should_split', False) else 'No'}")
    ...
    if not no_split:
        ...
        for i, (emb, name) in enumerate(zip(sub_embeddings, sub_names)):
            format_tensor_with_similarity(
                emb, results['initial_embedding'], name)
    ...
    if no_split:
        ...
        sub_embeddings = results.get('sub_embeddings', [])
        total_memory = sum(emb.numel() * emb.element_size()
